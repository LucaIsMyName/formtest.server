...
export class FormFieldDetector {
  ...
  private async detectSubmitButtons(): Promise<string[]> {
    ...
    const patterns = ['button[type="submit"]', 'input[type="submit"]', 'button:has-text("Spenden")', 'button:has-text("Jetzt spenden")', 'button:has-text("Weiter")', 'button:has-text("Absenden")', 'button:has-text("Submit")', ".submit-btn", ".submit-button", "#submit", "[data-submit]"];
  ...
  private async analyzePaymentMethods(): Promise<{ hasPaymentSection: boolean; detectedMethods: string[] }> {
    ...
    try {
      ...
      const paymentSelectors = ['input[name*="payment"]', 'select[name*="payment"]', 'input[name*="zahlart"]', "[data-payment]", ".payment-method", ".zahlungsart"];
...
export class SmartFormFiller {
  ...
  private findCustomValue(field: FormField, customData: Record<string, string>): string | undefined {
    // Try different keys
    const keys = [field.name, field.id, field.label?.toLowerCase(), field.placeholder?.toLowerCase()].filter(Boolean);
  ...
  private analyzeFieldPurpose(field: FormField): { purpose: string; confidence: number } {
    const indicators = [field.name?.toLowerCase(), field.id?.toLowerCase(), field.label?.toLowerCase(), field.placeholder?.toLowerCase()].filter(Boolean).join(" ");
