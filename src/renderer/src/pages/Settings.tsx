                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3">
                        <h4 className="text-sm font-medium text-gray-900 dark:text-white">{getSettingDisplayName(setting.key)}</h4>
                        {editingKey !== setting.key && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-200">{formatValue(setting.key, setting.value)}</span>}
                      </div>

                      {setting.description && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{setting.description}</p>}

                      {getSettingHelp(setting.key) && <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{getSettingHelp(setting.key)}</p>}
